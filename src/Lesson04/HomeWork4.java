...
public class HomeWork4 {
  ...
  /*  private static boolean isWin(char symbol) {
    ...
    public static boolean chekDiagonal(char symbol){
        int firstDiagonal = 0;
        int secondDiagonal = 0;
        for (int i = 0; i < SIZE; i++) {
            firstDiagonal = (map[i][i] == symbol) ? firstDiagonal + 1 : 0;
            secondDiagonal = (map[i][map.length - 1 - i] == symbol) ? secondDiagonal + 1 : 0;
            if (firstDiagonal == WIN || secondDiagonal == WIN){
                return true;
            }
        }
        return false;
